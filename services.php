...
<head>
    ...
    <title>vocabill ERP - Υπηρεσίες</title>
...
<body class="bg-gray-100 h-screen">
    ...
    <div x-data="{ open: false }" class="fixed top-0 z-40 w-full bg-white shadow-md">
        <div class="flex flex-col md:flex-row md:justify-between md:items-center px-8 py-4">
            ...
            <nav class="hidden md:flex space-x-4 items-center mt-4 md:mt-0">
                ...
                <a href="services.php" class="text-gray-800 hover:text-gray-900 font-bold text-base">Υπηρεσίες</a>
        ...
        <main class="mt-8">
            ...
            switch ($action) {
                case 'list':
                    ...
                    break;
                case 'show':
                    $serviceController->showServiceDetails($id);
                    break;
                case 'create':
                    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                        $serviceController->createService($_POST);
                    } else {
                        $serviceController->createServiceForm();
                    }
                    break;
                case 'edit':
                    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                        $serviceController->updateService($id, $_POST);
                    } else {
                        $serviceController->editServiceForm($id);
                    }
                    break;
                case 'delete':
    ...
    <script>
        function startVoiceRecognition() {
            ...
            if ('webkitSpeechRecognition' in window) {
                ...
                recognition.onresult = function(event) {
                    ...
                    } else if (voiceCmd.toLowerCase() === "νέο τιμολόγιο") {
                        window.location.href = 'invoices.php?action=create';
                    } else if (voiceCmd.toLowerCase() === "αποθήκευση") {
                    ...
                    } else if (voiceCmd.toLowerCase().includes("εισαγωγή ")) {
        ...
        function fillInvoiceFormByVoice(voiceCmd) {
            ...
            } else if (cmd.includes("πιστωτική κάρτα")) {
                const paymentSelect = document.getElementById('payment_method');
                if (paymentSelect) {
                    for (let i = 0; i < paymentSelect.options.length; i++) {
                        if (paymentSelect.options[i].text.toLowerCase().includes("πιστωτική κάρτα")) {
                            paymentSelect.selectedIndex = i;
                            break;
                        }
                    }
                }
            } else if (cmd.includes("τραπεζική μεταφορά")) {
                const paymentSelect = document.getElementById('payment_method');
                if (paymentSelect) {
                    for (let i = 0; i < paymentSelect.options.length; i++) {
            ...
            } else if (cmd.includes("σε εκκρεμότητα")) {
                const statusSelect = document.getElementById('status');
                if (statusSelect) {
                    for (let i = 0; i < statusSelect.options.length; i++) {
